...
var AudioEngine = {
    FADE_TIME: 1,
    AUDIO_BUFFER_CACHE: {},
    _fileProtocolWarningShown: false,
    _disabled: false,
    _audioContext: null,
    _master: null,
    ...
    _getMissingAudioBuffer: function () {
        if (!AudioEngine._audioContext) {
            return null;
        }
        // silent fallback to avoid harsh beeps when audio can't be loaded
    ...
    _playSound: function (buffer) {
        if (AudioEngine._disabled || !AudioEngine._audioContext || !buffer) {
            return;
        }
    ...
    _stopEventMusic: function () {
        if (AudioEngine._disabled || !AudioEngine._audioContext) {
            return;
        }
        var fadeTime = AudioEngine._audioContext.currentTime + AudioEngine.FADE_TIME * 2;
    ...
    loadAudioFile: function (src) {
        if (AudioEngine._disabled || window.location.protocol === 'file:') {
